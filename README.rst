...
- Practical approach: compared to other prevailing hobby/course projects, this
  ...
  program built by cargo/rustc. It currently supports:
  ...
  - Interrupts (timer interrupt controlled by ``mtime``/``mtimecmp``, software
    interrupt that could be writing to a memory-mapped register and external
    interrupt with an ``irq`` pin, which could be interfaced further or triggered
    directly in Verilator simulator by keyboard input.
  - Console output to simulator through a memory-mapped register
  - Video framebuffer to simulator
...
- TODO: a semi-host example on Spartan-6/Artix-7


It Currently Does NOT Have...
=============================

- Cache implementation or DDR RAM interfacing implementation: this is more or
  less decoupled from the core implementation of the processor and it is also
  an interesting and deep topic by itself. I may would like to work on a
  modularized implementation to integrate with the core in the future, but
  lacking this does not undermine the main purpose (and fun) of this project.
...
- Supervisor/User mode: I do not plan to do this for now as it could significantly
  complicate the existing simple CSR implementation, which goes against the
...
- Rust Hello World: ``./sim -l apps/mriscv-rs/hello.bin=0x100000`` (make
  ``sim`` with ``ENABLE_SDL=1`` and use ``-v`` if want to test keyboard
  interrupts, press Esc to quit the UI)
